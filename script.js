document.addEventListener("DOMContentLoaded", () => {
    ...
    let currentGameName = null; // Store game name for display
    let guessCount = 0;
    let gaveUp = false;
        ...
        if (privateList) {
            ...
            if (!privateGames.length) {
                ...
                privateGames.forEach(game => {
                    ...
                    console.log(`Game Number (internal): ${gameNumber}`); // Log composite key internally
    ...
    function setupHints() {
        ...
        document.getElementById("current-game-number").textContent = currentGameNumber.includes("P") ? `Game - ${currentGameName}` : `Game #${currentGameNumber}`;
    }
    ...
    function endGame(won, gaveUp = false) {
        ...
        const displayGameName = currentGameNumber.includes("P") ? `Game - ${currentGameName}` : `Game #${currentGameNumber}`;
        gameNumberSpan.textContent = displayGameName;
        ...
        if (won) {
            ...
            shareGameNumber.textContent = displayGameName;
            shareScoreLabel.style.display = "none";
            shareScore.style.display = "none";
        } else {
            endGraphic.src = document.body.classList.contains("dark-mode") ? "sad_pineapple_dark.png" : "sad_pineapple_light.png";
    ...
    function loadGame(game) {
        ...
        currentGameName = game["Game Name"] || null; // Store game name for private games
        secretWord = game["Secret Word"].toUpperCase();
        hints = [
            game["Hint 1"], game["Hint 2"], game["Hint 3"],
            game["Hint 4"], game["Hint 5"]
        ].filter(hint => hint).map(hint => hint.toUpperCase());
        console.log("Loaded game:", { currentGameNumber, currentGameName, secretWord, hints });
        setupHints();
    }
